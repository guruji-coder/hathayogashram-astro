...
function generateStaticUrls(): Array<SitemapUrl> {
  ...
  return staticPages.map(page => ({
    ...
    alternates: INTERNATIONAL_LOCATIONS.map(loc => ({
      hreflang: loc.country === 'uk' ? 'en-GB' : 
                loc.country === 'usa' ? 'en-US' :
                loc.country === 'canada' ? 'en-CA' :
                loc.country === 'australia' ? 'en-AU' :
                loc.country,
...
${allUrls.map(({ url, lastmod, changefreq, priority, alternates }: SitemapUrl) => `
  <url>
    <loc>${url}</loc>
    <lastmod>${lastmod}</lastmod>
    <changefreq>${changefreq}</changefreq>
    <priority>${priority}</priority>${alternates ? alternates.map((alt: { hreflang: string; href: string }) => `

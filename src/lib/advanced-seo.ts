...
export function generateAdvancedMetaTags(config: AdvancedSEOConfig): MetaData[] {
  ...
  const baseTags: MetaData[] = [
    ...
    { name: "classification", content: "Education, Health, Wellness, Spiritual Development" },
    ...
    { name: "abstract", content: "Authentic yoga teacher training programs in Rishikesh, India" },
    
    // Target audience
    { name: "target-audience", content: "Yoga practitioners, aspiring teachers, wellness seekers" },
    { name: "intended-audience", content: "Adults interested in yoga education" },
    
    // Certification and accreditation
    { name: "certification", content: "Yoga Alliance RYS 200/300 Registered School" },
    { name: "accreditation", content: "Yoga Alliance USA" },
    
    // Course-specific if applicable
    ...(courseLevel ? [
      { name: "course-level", content: courseLevel },
      { name: "education-level", content: courseLevel === '200hour' ? 'Foundational' : 'Advanced' }
    ] : []),
    
    // Seasonal content if provided
    ...(season ? [
      { name: "seasonal-content", content: `${season} yoga programs` },
      { name: "seasonal-keywords", content: `${season} yoga training, ${season} retreat` }
    ] : []),
...
export function generateYogaOpenGraphTags(config: AdvancedSEOConfig & { 
  title: string; 
  description: string; 
  image?: string;
  url?: string;

...
<div class="mx-auto grid w-full max-w-5xl grid-cols-1 gap-10 md:grid-cols-3 md:px-8">
	{#each cards as card, index (card.title)}
		...
		>
			<img src={card.src} alt={card.title} class="absolute inset-0 object-cover" />
			<div
				class={cn(
			...
			>
				<div
					class="bg-gradient-to-b from-neutral-50 to-neutral-200 bg-clip-text text-xl font-medium text-transparent md:text-2xl"
				>
					{card.title}
				</div>
			</div>
